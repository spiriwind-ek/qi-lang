...
class Parser:
    ...
    def _parse_block_body(self):
        ...
        只在当前块层级消费 。——嵌套块的 。 由各自的 _parse_block_body 处理。
        """
    ...
    def parse_for_each(self):
        """对 列表 每项 X 于：statement"""
        self.expect(TokenType.FOR_EACH)
        list_tok = self.expect(TokenType.IDENT)
        self.expect(TokenType.EACH)
        item_tok = self.expect(TokenType.IDENT)
        ...
        if pt == TokenType.IDENT:
            ...
            if self.at(TokenType.MEMBER_ACCESS):
                ...
                if self.at(TokenType.IDENT):
                    member = self.expect(TokenType.IDENT)
                    if self.at(TokenType.LPAREN):
                        return MethodCall(name, member.value, self._parse_args())
