...
class Lexer:
    ...
    def tokenize(self):
        ...
        while self.pos < len(self.source):
            ...
            # 中文序列：按标点和单字符关键字（的、为）切分
            if ord(ch) > 127:
                SPLIT_CHARS = set(PUNCT_MAP.keys()) | {'的', '为', '含'}
                # 首字符是分隔符则直接处理
                ...
                # 否则读取连续中文序列直到遇到分隔符
                start_pos = self.pos
                while self.pos < len(self.source) and ord(self.peek()) > 127 and self.peek() not in SPLIT_CHARS:
                    self.advance()
                seq = self.source[start_pos:self.pos]

                i = 0
                while i < len(seq):
                    if seq[i] in SPLIT_CHARS:
                        if i > 0:
