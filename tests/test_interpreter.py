...
def test_func_decl_and_call():
    """令 整数 求和 为（整数甲、整数乙）：返回 甲加乙。"""
    code = ('令 整数 求和 为（整数甲、整数乙）：\n'
            '    返回 甲加乙。\n'
            '令 整数 总计 为 求和（3、5）。\n'
            '输出 总计。')
    interp = Interpreter()
...
def test_list_length():
    """库存之长度 应返回 3"""
    code = '令 整数列 库存 为 [10、20、30]。输出 库存之长度。'
    interp = Interpreter()
    result = interp.run(code)
    assert result == ['3'], f"Expected ['3'], got {result}"
...
def test_list_full_example():
    """完整示例：声明列表、访问元素、获取长度"""
    code = '令 整数列 库存 为 [10、20、30]。输出 库存[1]。输出 库存[2]。输出 库存[3]。输出 库存之长度。'
    interp = Interpreter()
    result = interp.run(code)
...
def test_struct_decl_and_member_access():
    """令 结构 学生 含：文本 姓名、整数 年龄。令 学生 小明 为 学生含（"小明"、18）。输出 小明之姓名。"""
    code = '令 结构 学生 含：文本 姓名、整数 年龄。令 学生 小明 为 学生含（"小明"、18）。输出 小明之姓名。'
    interp = Interpreter()
    result = interp.run(code)
...
def test_struct_member_access_int():
    """小明之年龄 应返回 18"""
    code = '令 结构 学生 含：文本 姓名、整数 年龄。令 学生 小明 为 学生含（"小明"、18）。输出 小明之年龄。'
    interp = Interpreter()
    result = interp.run(code)
