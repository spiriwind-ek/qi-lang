...
def test_turing_completeness():
    ...
    # 测试9：结构体（简化版）
    result = run_code('令 结构 点 含：整数 X、整数 Y。令 点 P 为 点含（3、4）。输出 P之X。')
...
def test_code_logic():
    ...
    assert result == ['10'], f"条件累加测试失败: {result}"
    print("  ✓ 条件累加")
    ...
    # 测试9：数组长度
    result = run_code('令 整数列 列表 为 [1、2、3、4、5]。输出 列表之长度。')
    assert result == ['5'], f"数组长度测试失败: {result}"
    print("  ✓ 数组长度")

    # 测试10：结构体访问
    result = run_code('令 结构 点 含：整数 X、整数 Y。令 点 P 为 点含（3、4）。输出 P之X。输出 P之Y。')
