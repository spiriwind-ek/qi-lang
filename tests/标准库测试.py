...
def test_文字长度():
    assert run('令 整数 X 为 文字库的长度("hello")。输出 X。') == ['5']

def test_文字截取():
    assert run('令 X 为 文字库的截取("hello"、1、3)。输出 X。') == ['hel']

def test_文字查找():
    assert run('令 整数 X 为 文字库的查找("hello"、"e")。输出 X。') == ['2']

def test_文字替换():
    assert run('令 X 为 文字库的替换("hello"、"e"、"a")。输出 X。') == ['hallo']

def test_转大写():
    assert run('令 X 为 文字库的转大写("hello")。输出 X。') == ['HELLO']

def test_去留白():
    assert run('令 X 为 文字库的去留白("  hi  ")。输出 X。') == ['hi']
