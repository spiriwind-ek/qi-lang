...
def test_stdlib_basic():
    assert run_vm('输出 文本库的长度("hello")。') == ['5']
    print("  ✓ test_stdlib_basic")

# ─── Bug 修复验证 ───

def test_func_body_while():
    """BUG-01: 函数体内支持循环"""
    assert run_vm('令 空 测试 为（）：重复 3 次：输出 "好"。测试（）。') == ['好', '好', '好']
    print("  ✓ test_func_body_while")

def test_is_truthy_zero():
    """BUG-06: 0 为假值"""
    assert run_vm('若 0 则：输出 "真"。否则：输出 "假".') == ['假']
    assert run_vm('若 1 则：输出 "真"。否则：输出 "假".') == ['真']
    print("  ✓ test_is_truthy_zero")

def test_is_truthy_empty():
    """BUG-06: 空文本/空列表为假值"""
    assert run_vm('若 "" 则：输出 "真"。否则：输出 "假".') == ['假']
    assert run_vm('若 "x" 则：输出 "真"。否则：输出 "假".') == ['真']
    print("  ✓ test_is_truthy_empty")

def test_output_not_accumulate():
    """BUG-02: 多次 run 不累积输出"""
    interp = Interpreter(use_vm=True)
    assert interp.run('输出 "A"。') == ['A']
    assert interp.run('输出 "B"。') == ['B']  # 不含 'A'
    print("  ✓ test_output_not_accumulate")

def test_short_circuit_and():
    """BUG-07: 且短路推 True/False"""
    assert run_vm('输出 假 且 真。') == ['False']
    assert run_vm('输出 真 且 假。') == ['False']
    assert run_vm('输出 真 且 真。') == ['True']
    print("  ✓ test_short_circuit_and")

def test_short_circuit_or():
    """BUG-07: 或短路推 True/False"""
    assert run_vm('输出 真 或 假。') == ['True']
    assert run_vm('输出 假 或 真。') == ['True']
    assert run_vm('输出 假 或 假。') == ['False']
    print("  ✓ test_short_circuit_or")
